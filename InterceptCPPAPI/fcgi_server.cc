...
handlePHPConnectionEvents(TSCont contp, TSEvent event, void *edata)
{
  ...
  case TS_EVENT_NET_CONNECT: {
    ...
    TSStatIntIncrement(fcgiGlobal::reqId, 1);
  ...
  case TS_EVENT_VCONN_EOS: {
    ...
    TSStatIntIncrement(fcgiGlobal::respId, 1);
    return TS_EVENT_NONE;
  }
...
void
FCGIServer::setDefaultLimits(int64_t min, int64_t max, int64_t queueSize)
{
  max_connections    = min;
  min_connections    = max;
...
FCGIServer::connect(FastCGIIntercept *intercept)
{
  ...
  char *arr  = fcgiGlobal::plugin_data->getGlobalConfigObj()->getServerIp();
  char *port = fcgiGlobal::plugin_data->getGlobalConfigObj()->getServerPort();
  cout << "arr : " << arr << endl;
...
void
FCGIServer::addToRequestQueue(FastCGIIntercept *intercept)
{
  request_queue.push(intercept);
}
FastCGIIntercept *
FCGIServer::popFromRequestQueue()
{
  FastCGIIntercept *intercept;
  if (!request_queue.empty()) {
    intercept = request_queue.front();
    request_queue.pop();
...
// void
// interceptStaticTransferData(InterceptIO *server, FCGIClientRequest *fcgiRequest)
// {
//   TSIOBufferBlock block;
//   int64_t consumed       = 0;
//   server->serverResponse = "";
...
// static int
// handleStaticContentEvents(TSCont contp, TSEvent event, void *edata)
// {
//   TSDebug(PLUGIN_NAME, "HandlePHPConnectionEvents:  event( %d )\tEventName: %s", event, TSHttpEventNameLookup(event));
...
//     // Increment the statistic as time passes.
//     TSStatIntIncrement(fcgiGlobal::reqId, 1);
//     TSDebug(PLUGIN_NAME, "Connected to FCGI Server. WriteIO.vio :%p \t ReadIO.vio :%p ", server->writeio.vio,
//     server->readio.vio);
//     break;
//   }
...
//   case TS_EVENT_VCONN_READ_READY: {
//     TSDebug(PLUGIN_NAME, "HandlePHPConnectionEvents: Inside Read Ready...VConn Open ");
//     interceptStaticTransferData(server, fcgiRequest);
...
// const uint
// FCGIServer::connectStatic(FastCGIIntercept *intercept)
// {
//   const uint request_id = UniqueRequesID::getNext();
...
//   // contp is a global netconnect handler  which will be used to connect with
//   // php server
//   contp = TSContCreate(handleStaticContentEvents, TSMutexCreate());
//   TSContDataSet(contp, new RequestIntercept(request_id, this));
//   intercept->contp_ = contp;
//   TSNetConnect(contp, (struct sockaddr const *)&ip_addr);
//   TSDebug(PLUGIN_NAME, "FastCGIIntercept::initServer : Data Set Contp: %p", contp);
