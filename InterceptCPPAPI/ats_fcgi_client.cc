...
//input to the constructor will be either unique transaction id or int type requestId
...
FCGIClientRequest::createBeginRequest(std::map<std::string,std::string> fcgiReqHeaders){
    ...
    TSDebug(PLUGIN_NAME,"ParamsLen: %d ContLenB0: %d ContLenB1: %d",len,state_->header->contentLengthB0,state_->header->contentLengthB1);
    ...
    if(str.compare(fcgiReqHeaders["REQUEST_METHOD"])==0){
        ...
        serialize(state_->pBuffInc,state_->postHeader, sizeof(FCGI_Header));
        state_->pBuffInc +=sizeof(FCGI_Header);
        TSDebug(PLUGIN_NAME,"Post Header Len: %d ",state_->pBuffInc - state_->buff);
    }
...
unsigned char* FCGIClientRequest::addClientRequest(string data,int &dataLen,std::map<std::string,std::string> fcgiReqHeaders){
...
uint32_t FCGIClientRequest::serializeNameValue(uchar* buffer, std::map<string,string>::iterator it){
     uchar *p = buffer;
     uint32_t nl, vl;
    nl = it->first.length();
    vl = it->second.length();
...
uint32_t FCGIClientRequest::fcgiHeaderGetContentLen(FCGI_Header *h){
    printf("\ncontentLengthB1: %d ,contentLengthB0 : %d ,After shieft content_len_hi: %d",h->contentLengthB1,h->contentLengthB0,(h->contentLengthB1 << 8) );
...
int FCGIClientRequest::fcgiProcessContent(uchar **beg_buf, uchar *end_buf,FCGIRecordList *rec){
...
int FCGIClientRequest::fcgiProcessRecord(uchar **beg_buf, uchar *end_buf, FCGIRecordList *rec){
...
void FCGIClientRequest::fcgiProcessBuffer(uchar *beg_buf, uchar *end_buf,FCGIRecordList** head){
